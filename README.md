# stock — 주식 트레이딩 모노레포

GitHub: [zzatomi02/stock_ai](https://github.com/zzatomi02/stock_ai)

## 사용 설명서 (docs)

| 문서 | 내용 |
|------|------|
| [docs/README.md](./docs/README.md) | 문서 목록·보는 방법 |
| [docs/GETTING-STARTED.md](./docs/GETTING-STARTED.md) | Docker / 로컬 실행 |
| [docs/TRADING-FLOW.md](./docs/TRADING-FLOW.md) | 장전·장중·장마감 파이프라인 |
| [docs/AI-PLATFORM.md](./docs/AI-PLATFORM.md) | AI 배치·캐시·API |
| [docs/GIT.md](./docs/GIT.md) | GitHub 계정 (`zzatomi02`) |

## 프로젝트

| 프로젝트 | 역할 | README |
|----------|------|--------|
| `stock-trading-frontend-v4` | Next.js 15 UI | [README](./stock-trading-frontend-v4/README.md) |
| `stock-trading-backend-v4` | Spring Boot API | [README](./stock-trading-backend-v4/README.md) |
| `stock-trading-ai-service-v4` | FastAPI AI | [README](./stock-trading-ai-service-v4/README.md) |

빠른 시작은 [docs/GETTING-STARTED.md](./docs/GETTING-STARTED.md) 를 보세요.
