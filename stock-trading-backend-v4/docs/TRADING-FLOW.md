...
| 프로젝트 | 역할 |
|---------|------|
| stock-trading-backend-v4 | 전략·리스크·뉴스·AI·스케줄·DB |
| stock-trading-frontend-v4 | UI (AI 분석 조회 등) |
| stock-trading-ai-service-v4 | (선택) 외부 AI 프록시 — `app.ai.base-url` |
...
뉴스가 ingest 되면 **즉시** `IntradaySignalPipelineService` → `strategy_signal` 생성 (API 호출 없음).
...
API Key는 DB에 저장하지 않음 → `ai_provider_config.api_key_env_name` + 환경 변수 (`OPENAI_API_KEY`).

---

## 6. 로컬 첫 실행 체크리스트

1. MySQL + `mvn spring-boot:run` (profile `local`)
2. `OPENAI_API_KEY` (실제 AI 호출 시)
3. `POST /api/watchlist` + `X-User-Id: system` 으로 관심종목 등록
4. `POST /api/trading-flow/pre-market/run`
5. `GET /api/ai/company-analysis/today` 확인
6. 뉴스 수집 또는 `POST /api/strategy/analysis/article/{id}`
7. `GET /api/strategy/analysis/signals` — `aiBlendedScore`, `riskCheckPassed` 확인
...
- ingest `SignalEngine` vs `StrategyAnalysisService` 이중 경로 정리
- 프론트 시그널 상세 UI
...
AI `RISK`/`AVOID` → 매수 후보 제외. Risk Engine 최종 주문 단계에서도 재검증.

전략별 fallback: `StrategyAiPolicyService` 참고.
